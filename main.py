...
async def cmd_stopbatch(client, message):
    ...
    total = len(anime_db.get(anime_name, []))

    save_db(anime_name)

    await message.reply_text(
        f"✅ Stop Batch Successfully!\n\n"
        f"🎬 Anime: {anime_name.title()}\n"
        f"📦 Total Episodes Saved: {total}\n\n"
        f"✅ All Anime Added Successfully!"
    )

# =========================================
# ADMIN: /delete anime_name
# =========================================
...
async def cmd_delete(client, message):
    ...
    found = find_anime(parts[1])
    if not found:
        return await message.reply_text(f"❌ '{parts[1]}' nahi mila database mein.")

    ep_count = len(anime_db[found])
    del anime_db[found]
    anime_col.delete_one({"name": found})

    await message.reply_text(
        f"🗑️ Deleted Successfully!\n\n"
        f"🎬 Anime: {found.title()}\n"
        f"📦 Episodes removed: {ep_count}"
    )
...
async def cmd_stats(client, message):
    if message.from_user.id != ADMIN_ID:
        return await message.reply_text("❌ Sirf admin dekh sakta hai.")

    total_anime = len(anime_db)
    total_eps = sum(len(v) for v in anime_db.values())
    total_users = users_col.count_documents({})
    active_sends = len(send_mode)
    ...
    await message.reply_text(
        ...
        f"⚡ Active Sends: {active_sends}\n"
        f"🧠 AI Sessions: {len(chat_memory)}"
    )
...
async def cmd_broadcast(client, message):
    ...
    for u in all_users:
        try:
            await client.send_message(u["user_id"], f"📢 Rei ka Message:\n\n{bcast_text}")
            sent += 1
...
# =========================================
# SAVE FORWARDED EPISODES (admin in batch)
# =========================================
...
async def save_episodes(client, message):
    ...
    batch = save_mode[message.chat.id]
    anime_name = batch["name"]
    ...
    await message.reply_text(
        f"✅ Ep {total} Saved! — {anime_name.title()}"
    )
...
EXCLUDED_CMDS = [
    "start", "batch", "stopbatch", "send", "stop",
    "continue", "list", "search", "status", "ep",
    "clear", "delete", "stats", "broadcast"
]
...
async def main_chat(client, message):
    try:
        ...
        reply = await ask_ai_async(str(uid), user_text)
        ...
        print("Chat error:", e)
        traceback.print_exc()
        await message.reply_text("❌ Error aa gaya, thoda baad try karo.")
